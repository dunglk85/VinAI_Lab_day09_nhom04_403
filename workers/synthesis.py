...
def _call_llm(messages: list) -> str:
    ...
    Thử OpenAI trước (nếu có key), sau đó Gemini.
    Sprint 2: Đã implement — tự chọn provider dựa trên API key có sẵn.
    """
    ...
    # Option A: OpenAI (chỉ thử nếu key có vẻ hợp lệ)
    ...
    # Option B: Gemini (chỉ thử nếu key có vẻ hợp lệ, có retry cho rate limit)
            ...
            for attempt in range(3):
                try:
                    ...
                    if "429" in err_str and attempt < 2:
                        ...
                        print(f"  ⏳ Gemini rate limit ({model_name}), retry in {wait}s...")
                        time.sleep(wait)
                    else:
                        errors.append(f"Gemini({model_name}): {e}")
                        break

    # Fallback: trả lời dựa trên context thuần (rule-based, không hallucinate)
    if errors:
        print(f"  ⚠️  LLM errors: {errors}")
    return _fallback_synthesis(messages)


def _fallback_synthesis(messages: list) -> str:
    """
    Fallback khi không có LLM key: trích xuất context từ messages
    và tạo câu trả lời rule-based có citation.
    Không hallucinate — chỉ trả về text từ context.
    """
    user_msg = ""
    for m in messages:
        if m.get("role") == "user":
    ...
    if not user_msg or "(Không có context)" in user_msg:
        return "Không đủ thông tin trong tài liệu nội bộ để trả lời câu hỏi này."

    # Trích xuất phần câu hỏi và context
    ...
    for line in lines:
        ...
        elif "TÀI LIỆU THAM KHẢO" in line or "POLICY EXCEPTIONS" in line:
            in_context = True
    ...
    # Tạo câu trả lời từ context (giữ nguyên text gốc + citation)
    answer_parts = [f"Dựa trên tài liệu nội bộ:\n"]
    for line in context_lines:
        if line.startswith("[") or line.startswith("Nguồn:") or line.startswith("-"):
            continue
        if line.startswith("Hãy trả lời"):
            continue
        if len(line) > 10:
