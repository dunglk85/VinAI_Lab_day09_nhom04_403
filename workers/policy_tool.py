...
def run(state: dict) -> dict:
    ...
    try:
        ...
        if needs_tool:
            if any(kw in task.lower() for kw in ["ticket", "p1", "jira"]):
                mcp_result = _call_mcp_tool("get_ticket_info", {"ticket_id": "P1-LATEST"})
                state["mcp_tools_used"].append(mcp_result)
                state["history"].append(f"[{WORKER_NAME}] called MCP get_ticket_info")

            if "access" in task.lower() or "cấp quyền" in task.lower():
                # Giả định level từ task (demo)
                level = 3 if "level 3" in task.lower() else 1
                mcp_result = _call_mcp_tool("check_access_permission", {
                    "access_level": level,
                    "requester_role": "contractor" if "contractor" in task.lower() else "staff",
                ...
                # Merge MCP result into policy_result
                if mcp_result.get("output"):
                    policy_result.update({
                        "mcp_access_check": mcp_result["output"],
                        "policy_applies": mcp_result["output"].get("can_grant", policy_result["policy_applies"])
                    })
